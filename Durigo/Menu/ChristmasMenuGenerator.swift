//
//  ChristmasChristmasMenuGenerator.swift
//  Durigo
//
//  Created by Joshua Cardozo on 24/12/23.
//

import SwiftUI

extension ChristmasMenuGenerator {
                        ...
                        HStack(alignment: .top, spacing: 0) {
                            VStack(spacing: 0) {
                                ...
                                ForEach(left) { category in
                                    ChristmasMenuGenerator.MenuCategory(name: category.name, fontSize: categoryFontSize)
                                    ForEach(category.items.filter({ [Category.Item.VisibilityScope.menu, Category.Item.VisibilityScope.both].contains($0.visibilityScope)})) { item in
                                        ChristmasMenuGenerator.MenuItem(item: item, fontSize: itemFontSize, descriptionFontSize: itemDescriptionFontSize, topPadding: itemTopPadding)
                                    }
                                }
...
struct ChristmasMenuGenerator: View {
